...
const roleIdle = { ...roleBase, border: \1px solid \\, color: inkFaint };
...
export default function Login() {
  ...
  return (
    <div style={{ display: "flex", width: "100%", minHeight: "100vh", background: "#07080a", fontFamily: "'Noto Sans TC',sans-serif" }}>
      <div style={{ position: "relative", flex: 1.15, minWidth: 0, overflow: "hidden", borderRight: \1px solid \\ }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 40, background: "radial-gradient(600px 500px at 70% 0%,#0c0d0f,#050506)" }}>
        <div style={{ width: "100%", maxWidth: 400 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            <span style={{ width: 8, height: 8, border: \1px solid \\ }} />
          ...
          <h1 style={{ fontFamily: "'Chakra Petch',sans-serif", color: ink, fontSize: 26, margin: "14px 0 4px", fontWeight: 600 }}>歡迎回到艦橋</h1>
          <p style={{ color: inkFaint, fontFamily: "'Space Mono',monospace", fontSize: 12, margin: "0 0 26px", letterSpacing: ".08em" }}>AUTHENTICATE TO BOARD</p>

          <div style={{ display: "flex", gap: 4, padding: 4, border: \1px solid \\, borderRadius: 2, marginBottom: 24 }}>
            <button style={activeTab}>登入</button>
            <button onClick={() => navigate("/register")} style={idleTab}>註冊</button>
          </div>

          {errorMsg && <div style={{ padding: 12, marginBottom: 16, border: \1px solid \\, color: errorMsg.includes("成功") ? steel : "#b08585", fontSize: 13, borderRadius: 2 }}>{errorMsg}</div>}
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setRole("user")} style={isCreator ? roleIdle : roleActive}>
              ...
              <button onClick={() => setRole("admin")} style={isCreator ? roleActive : roleIdle}>
          ...
          <button onClick={submit} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", height: 50, background: "rgba(255,255,255,.06)", color: ink, fontFamily: "'Space Mono',monospace", fontWeight: 400, fontSize: 13, border: \1px solid \\, borderRadius: 2, letterSpacing: ".16em", textTransform: "uppercase", cursor: "pointer" }}>登入艦橋 ▸</button>
          <p style={{ textAlign: "center", color: inkFaint, fontSize: 11.5, marginTop: 18, fontFamily: "'Space Mono',monospace", letterSpacing: ".04em" }}>{isCreator ? "造物者將進入控制台 · CREATOR CONSOLE" : "探索家將進入星圖 · STAR CHART"}</p>
        </div>
      </div>
    </div>
  );
